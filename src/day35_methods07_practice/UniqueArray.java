...
public class UniqueArray {
	public static void main(String[] args) {
		...
		findUnique(arr);
		int[] arr1 = {2,3,4,0,5,6,7,2,3,4,5,1};
		getUniqueArray(arr1);
		int[] arr2 = {10,4,5,6,4,5};
		getUniqueArray(arr2);
		getUniqueArrayCheapV2(arr2);
	}

	public static void findUnique(int[] nums) {//outer loop
		for (int i = 0; i < nums.length; i++) {
			int temp = nums[i]; // get a number
			int counter = 0;
			//
			for (int j = 0; j < nums.length; j++) { //inner/nested loop
				if (nums[j] == temp && i != j) {// i ! = j is used to make sure we are not comparing values from the same loop
					counter++;
					break;
				}
			}
